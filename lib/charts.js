...
export async function Choropleth(data, {
    ...
    // Compute values.
    const N = d3.map(data, id); // array of neighborhoods
    const V = d3.map(data, value).map(d => d == null ? NaN : +d); // array of values to be represented
    ...
    const projectionTest = d3.geoIdentity().reflectY(true)
        .fitSize([width, height], features);
    // Construct a path generator.
    const path = d3.geoPath(projectionTest);
...
// export async function ChoroplethChoroplethWithDots(data, {
//
// }